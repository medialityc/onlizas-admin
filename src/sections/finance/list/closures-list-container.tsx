...
import { ClosuresList } from "./closures-list";
...
export default function ClosuresListContainer({
  closuresPromise,
  query,
}: ClosuresListContainerProps) {
  const { updateFiltersInUrl } = useFiltersUrl();

  // Local state for date filters (string ISO yyyy-mm-dd)
  const [fromDate, setFromDate] = useState<string>(query.fromDate || "");
  const [toDate, setToDate] = useState<string>(query.toDate || "");

  // Preserve original query values when DataGrid triggers changes (search, pagination, sorting)
