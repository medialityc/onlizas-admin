...
export function WarehouseCard ({ warehouse, onEdit, onDelete }: WarehouseCardProps) {
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6 flex gap-4 flex-col shadow-sm hover:shadow-md transition-shadow">
      {/* Header con icono y estado */}
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          <div className={clsx(
            'p-3 rounded-lg',
            warehouse.type === 'physical' ? 'bg-purple-100 text-purple-600' : 'bg-blue-100 text-blue-600'
          )}>
            <TypeIcon className="h-6 w-6" />
          </div>
          <div>
            <h3 className="text-lg font-semibold text-gray-900 hover:text-primary cursor-pointer" onClick={handleView}>
              {warehouse.name}
            </h3>
        ...
        {warehouse.location?.address && (
          <div className="flex items-start gap-2 text-sm text-gray-600">
            ...
            <div>
              ...
              <div className="text-gray-500">{warehouse.location.region}, {warehouse.location.country}</div>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          {warehouse.maxCapacity && (
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-xs font-medium text-gray-700">{warehouse.currentCapacity || 0} / {warehouse.maxCapacity}</span>
              </div>
              ...
              <div className="text-xs text-gray-500 mt-1">{warehouse.occupancyPercentage || 0}% ocupado</div>
            </div>
          )}
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-xl font-semibold text-gray-900">{getSupplierCount()}</span>
            </div>
      ...
      <div className="flex gap-2 mt-4 pt-4 border-t border-gray-100">
        <button
          className="flex-1 py-2 px-3 rounded border border-gray-300 text-gray-700 text-sm font-medium hover:bg-gray-50 transition"
          onClick={handleEdit}
        >
          Editar
        </button>
