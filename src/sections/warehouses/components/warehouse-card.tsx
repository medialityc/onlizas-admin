...
export function WarehouseCard({
  ...
  const getTypeIcon = () => {
    return warehouse.type === "physical"
      ? BuildingStorefrontIcon
      : BuildingOfficeIcon;
  };
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 flex gap-4 flex-col shadow-sm hover:shadow-md dark:hover:shadow-gray-900/20 transition-shadow h-full">
      ...
      <div className="flex items-start justify-between">
        ...
        <Badge className={clsx(getStatusColor(), "text-xs flex-shrink-0")}>
          {getStatusText()}
        </Badge>
      </div>
          ...
          <div className="flex items-start gap-2 text-sm text-gray-600 dark:text-gray-400">
            ...
            <div>
              ...
              <div className="text-gray-500 dark:text-gray-500">
                {warehouse.location.region}, {warehouse.location.country}
              </div>
            </div>
          </div>
        )}
            ...
            <div>
              <div className="flex items-center gap-2 justify-between mb-1">
                ...
                <span className="text-xs font-medium text-gray-700 dark:text-gray-300">
                  {warehouse.currentCapacity || 0} / {warehouse.maxCapacity}
                </span>
              </div>
              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                <div
                  className={clsx(
                    ...
                    (warehouse.occupancyPercentage || 0) >= 90
                      ? "bg-red-500"
                      : (warehouse.occupancyPercentage || 0) >= 70
                        ? "bg-yellow-500"
                        : "bg-green-500"
                  )}
                  style={{
                    width: `${Math.min(warehouse.occupancyPercentage || 0, 100)}%`,
                  }}
                ></div>
              </div>
              <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                {warehouse.occupancyPercentage || 0}% ocupado
              </div>
            </div>
          )}{" "}
          {/* Proveedores */}
