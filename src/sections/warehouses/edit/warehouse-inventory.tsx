  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-lg font-semibold text-gray-900">Inventarios del Almacén</h2>
        <SearchInput
          value={searchQuery}
        ...
        {filteredInventory.map((item) => (
          <div key={item.id} className="bg-white rounded-lg border border-gray-200 p-6">
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <div className="flex items-center gap-4 mt-1 text-sm text-gray-600">
                  <span>Categoría: <span className="text-blue-600">{item.category}</span></span>
                  <span>Proveedor: <span className="text-green-600">{item.supplier}</span></span>
            ...
            <div className="space-y-3">
              <h4 className="text-sm font-medium text-gray-700">Cantidades por variante:</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {item.variants.map((variant) => (
                  <div key={variant.id} className="bg-gray-50 rounded-lg p-4">
                    <div className="flex justify-between items-start">
                      <div>
                        <h5 className="font-medium text-gray-900">{variant.name}</h5>                        <div className="mt-1 text-sm text-gray-600">
                      ...
                      <div className="text-right">
                        <div className="text-lg font-semibold text-gray-900">{variant.available}</div>
                        <div className="text-sm text-gray-500">Disponible: {variant.available}</div>
                      </div>
                    </div>
                  </div>
                ))}
