...
export default function VendorRequestsTab({
  ...
  const existingIds = [
    ...(approvalProcess?.approvedCategories ?? []).map((c: { id: string | number }) =>
      String(c.id)
    ),
    ...(approvalProcess?.pendingCategories ?? []).map((c: { id: string | number }) =>
      String(c.id)
    ),
  ];

  // Hook centralizado para manejo de solicitudes de aprobación
  const { submitApprovalRequest, isLoading } = useApprovalProcessRequest(() => {
    setCategoryModalOpen(false);
    setExpirationModalOpen(false);
  });

  const handleApprovalRequest =
    (type: "category" | "expiration") => (data: any) => {
      submitApprovalRequest(user?.approvalProcessId, data, type);
    };
  ...
  return (
    <div className="space-y-6">
      ...
      <CategoryRequestModal
        ...
        onSubmit={handleApprovalRequest("category")}
        loading={isLoading}
      />
      ...
      <ExpirationExtensionModal
        ...
        onSubmit={handleApprovalRequest("expiration")}
        loading={isLoading}
