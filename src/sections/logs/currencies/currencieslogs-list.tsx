...
import { CurrenciesLogs, GetAllCurrenciesLogs } from "@/types/currencies";
...
function CurrenciesLogsContent({
  data,
  searchParams,
  onSearchParamsChange,
}: {
  data: GetAllCurrenciesLogs;
  searchParams: SearchParams;
  onSearchParamsChange: (params: SearchParams) => void;
}) {
  const [open, setOpen] = useState(false);
  const [selected, setSelected] = useState<CurrenciesLogs | null>(null);
  ...
  const handleRowClick = useCallback((rowOrWrapper: any) => {
    const row = extractRecord<CurrenciesLogs>(rowOrWrapper);
    console.log(row, "row");
    setSelected(row);
    setOpen(true);
  }, []);
  ...
  const grouped = useMemo(() => {
    const groups: Record<string, CurrenciesLogs[]> = {};
  ...
  const columns: DataTableColumn<CurrenciesLogs>[] = [
    {
      accessor: "timestamp",
      title: "Fecha/Hora",
      sortable: true,
    ...
    { accessor: "currencyName", title: "Moneda", sortable: true },
    { accessor: "codIso", title: "ISO", sortable: true },
    { accessor: "symbol", title: "Símbolo", sortable: false },
      ...
      {grouped.length > 0 ? (
        <div className="space-y-8">
          {grouped.map(([dayKey, rows]) => (
            <div key={dayKey} className="space-y-3">
              ...
              <DataGrid
                ...
                emptyText="No se encontraron logs de monedas"
          ...
          <SimpleModal
            ...
            title="Detalle del Log de Moneda"
            subtitle={
              selected
                ? `${selected.currencyName} (${selected.codIso}) · ${selected.userName || "-"}`
                : undefined
            }
            className="max-w-3xl"
          >
            {selected && (
              <div className="space-y-4 text-sm">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <div
                      ...
                      title={selected.currencyName}
                    >
                      {selected.currencyName}
                    </div>
                    <div className="text-xs text-gray-500">
                      ID: {selected.currencyId}
                    </div>
                  </div>
                  <div>
                    <div className="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">
                ...
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div className="truncate">ISO: {selected.codIso}</div>
                  <div className="truncate">Símbolo: {selected.symbol}</div>
                </div>
              </div>
            )}
          </SimpleModal>
...
export default CurrenciesLogsContent;
