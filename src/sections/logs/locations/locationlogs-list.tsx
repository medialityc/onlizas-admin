...
import { GetAllLocationLogsResponse, LocationLogs } from "@/types/locations";
...
function LocationLogsContent({
  data,
  searchParams,
  onSearchParamsChange,
}: {
  data: GetAllLocationLogsResponse;
  searchParams: SearchParams;
  onSearchParamsChange: (params: SearchParams) => void;
}) {
  const [open, setOpen] = useState(false);
  const [selected, setSelected] = useState<LocationLogs | null>(null);

  const handleRowClick = useCallback((row: LocationLogs) => {
    setSelected(row);
    setOpen(true);
  }, []);

  const grouped = useMemo(() => {
    const groups: Record<string, LocationLogs[]> = {};
  ...
  const columns: DataTableColumn<LocationLogs>[] = [
    {
      accessor: "timestamp",
      title: "Fecha/Hora",
      sortable: true,
    ...
    { accessor: "locationId", title: "ID", sortable: true },
    { accessor: "locationName", title: "Ubicación", sortable: true },
    { accessor: "locationAddress", title: "Dirección", sortable: false },
    { accessor: "locationType", title: "Tipo", sortable: true },
    {
      ...
      {grouped.length > 0 ? (
        <div className="space-y-8">
          {grouped.map(([dayKey, rows]) => (
            <div key={dayKey} className="space-y-3">
              ...
              <DataGrid
                ...
                emptyText="No se encontraron logs de ubicaciones"
          ...
          <SimpleModal
            ...
            title="Detalle del Log de Ubicación"
            subtitle={
              selected
                ? `${selected.locationName} · ${selected.locationAddress}`
                : undefined
            }
            className="max-w-3xl"
          >
              ...
              <div className="space-y-4 text-sm">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className="truncate" title={selected.locationAddress}>
                      {selected.locationAddress}
                    </div>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div className="truncate">Tipo: {selected.locationType || "N/A"}</div>
                  <div className="truncate">Estado: {selected.locationStatus}</div>
                </div>
...
export default LocationLogsContent;