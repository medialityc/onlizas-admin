...
export default function StoreEditContainer({ store, supplierId }: Props) {
  ...
  const banners = Array.isArray(data?.banners) ? data.banners : [];
      ...
      const logoValue = data.logoStyle instanceof File ? null : (typeof data.logoStyle === "string" ? data.logoStyle : store.logoStyle ?? "");
      if (data.logoStyle instanceof File) formData.append("logoStyle", data.logoStyle);
      ...
      const res = await updateStore(supplierId, store.id, formData);
      if (!res.error) {
