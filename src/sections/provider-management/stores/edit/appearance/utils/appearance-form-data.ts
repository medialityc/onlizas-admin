...
export function buildThemeFormData({
  store,
  data,
}: BuildAppearanceFormDataParams) {
...
type BuildBannersFormDataParams = {
  ...
  filter?: (
    banner: AppearanceForm["banners"][number],
    index: number,
  ) => boolean;
  isUpdate?: boolean;
};
...
export async function buildBannersFormData({
  banners,
  storeId,
  filter,
  isUpdate = false,
}: BuildBannersFormDataParams) {
  const formData = new FormData();
  ...
  const filteredBanners = banners.filter((b, idx) =>
    filter ? filter(b, idx) : true,
  );

  // Procesar banners para incluir imágenes en metadata si es update
  const processedBanners = await Promise.all(
    filteredBanners.map(async (b) => {
      const metadata: any = {
        title: b.title ?? "",
        urlDestinity: b.urlDestinity ?? "",
        position:
          typeof b.position === "number" ? b.position : Number(b.position ?? 0),
      ...
      // Para update, incluir imágenes en metadata SOLO si son File
      // Si son string (urls existentes), no las mandamos para no sobreescribir
      if (isUpdate) {
