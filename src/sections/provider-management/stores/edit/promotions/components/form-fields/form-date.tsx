...
export default function FormDate({ name = "simpleDates" }: { name?: string }) {
	...
	return (
		<div className="space-y-0">
			...
			<div className="space-y-0">
				{fields.length === 0 && (
					<div className="text-sm text-gray-500">No hay días específicos seleccionados</div>
				)}

				{fields.map((f, idx) => (
					<div key={f.id} className="flex items-start gap-2">
						<div className="space-y-0"><RHFDatePickerBanner name={`${name}.${idx}`} minDate={today} /></div>
						<Button
							type="button"
							iconOnly
							variant="secondary"
