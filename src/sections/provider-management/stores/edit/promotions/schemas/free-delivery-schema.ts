...
export const freeDeliverySchema = z.object({
  ...
  productVariantsIds: z
    .array(z.union([z.string().min(1), z.number().min(1)]))
  ...
  mediaFile: z.union([
    z.string().min(1, { message: "La imagen es obligatoria" }),
    z.instanceof(File, { message: "Seleccione una imagen" }),
  ]),

  // Estado
  active: z.boolean(),
  code: z.string({ error: "Debe establecer un código" }),
