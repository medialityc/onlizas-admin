...
import {
  createPromotionOvervalue,
  updatePromotionOvervalue,
} from "@/services/promotions";

export function usePromotionOvervalueMutations(storeId: string) {
  ...
  const createMutation = useMutation({
    ...
    onError: (err: any) =>
      toast.error(err?.message || "Error al crear promoción sobrevalor"),
  });
  ...
  const updateMutation = useMutation({
    ...
    onError: (err: any) =>
      toast.error(err?.message || "Error al actualizar promoción sobrevalor"),
  });
