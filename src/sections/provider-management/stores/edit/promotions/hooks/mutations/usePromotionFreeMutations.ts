...
export function usePromotionFreeMutations(storeId: string) {
  ...
  const createMutation = useMutation({
    ...
    onError: (err: any) =>
      toast.error(err?.message || "Error al crear promoción free"),
  });
  ...
  const updateMutation = useMutation({
    ...
    onError: (err: any) =>
      toast.error(err?.message || "Error al actualizar promoción free"),
  });
