...
export function usePromotionCodeMutations(storeId: string) {
  ...
  const createMutation = useMutation({
    ...
    onError: (err: any) =>
      toast.error(err?.message || "Error al crear promoción con código"),
  });
  ...
  const updateMutation = useMutation({
    ...
    onError: (err: any) =>
      toast.error(err?.message || "Error al actualizar promoción con código"),
  });
