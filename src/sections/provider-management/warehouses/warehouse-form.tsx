...
export function WarehouseForm({ warehouse, onSuccess }: WarehouseFormProps) {
  ...
  const mutation = useMutation({
    mutationFn: (data: WarehouseFormValues) => {
      ...
      return isEdit
        ? updateWarehouse(warehouse!.id, transformedData)
        : createWarehouse(transformedData);
    },
    onSuccess: () => {
    ...
    <FormProvider methods={methods} onSubmit={onSubmit}>
      <div className="space-y-5">
        ...
        <div className="flex gap-3 pt-2">
          <LoaderButton type="submit" loading={mutation.isPending}>
