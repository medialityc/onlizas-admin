...
import { CreateWarehouse, GetAllWarehouses, UpdateWarehouse, Warehouse, WarehouseFilter, WarehouseAuditLog } from '@/types/warehouses';
import { defaultVirtualRulesBySubType, mockVirtualWarehouses } from '@/data/werehouses/warehouses';
...
let warehouseAuditLog: WarehouseAuditLog[] = [];

const delay = (ms: number) => new Promise(resolve => setTimeout(resolve, ms));

// Helper para calcular permisos basado en reglas de negocio (simula BFF)
function calculateWarehousePermissions (warehouse: Warehouse): Warehouse['permissions'] {
...
function calculateWarehouseBusinessRules (warehouse: Warehouse): Pick<Warehouse, 'hasActiveInventory' | 'occupancyPercentage' | 'canBeDeleted' | 'canBeDeactivated'> {
  const hasInventory = (warehouse.currentCapacity || 0) > 0;
  const occupancy = warehouse.maxCapacity ? Math.round(((warehouse.currentCapacity || 0) / warehouse.maxCapacity) * 100) : 0;
...
export async function getAllWarehouses (params: IQueryable & WarehouseFilter): Promise<ApiResponse<GetAllWarehouses>> {
  ...
  if (params.search) {
    ...
    list = list.filter(warehouse => warehouse.name.toLowerCase().includes(searchTerm));
  }
  if (params.type) list = list.filter(warehouse => warehouse.type === params.type);
  if (params.status) list = list.filter(warehouse => warehouse.status === params.status);
  ...
  return { data: result, status: 200, error: false } as ApiResponse<GetAllWarehouses>;
}
...
export async function getWarehouseById (id: number): Promise<ApiResponse<Warehouse>> {
  ...
  if (!warehouse) return { error: true, status: 404, message: 'Not found' } as ApiResponse<Warehouse>;

  // Enriquecer con permisos calculados dinámicamente
  const enrichedWarehouse = enrichWarehouse(warehouse);

  return { data: enrichedWarehouse, status: 200, error: false } as ApiResponse<Warehouse>;
}

export async function createWarehouse (data: CreateWarehouse): Promise<ApiResponse<Warehouse>> {
  await delay(300);
  const id = mockWarehouses.length ? Math.max(...mockWarehouses.map(warehouse => warehouse.id)) + 1 : 1;
  ...
  return { data: enrichedWarehouse, status: 201, error: false } as ApiResponse<Warehouse>;
}
...
export async function updateWarehouse (id: number, data: UpdateWarehouse, audit: { reason?: string } = {}): Promise<ApiResponse<Warehouse>> {
  ...
  if (index === -1) return { error: true, status: 404, message: 'Not found' } as ApiResponse<Warehouse>;
...
export async function deleteWarehouse (id: number, audit: { reason?: string } = {}): Promise<ApiResponse<{ success: boolean }>> {
  await delay(200);
  const index = mockWarehouses.findIndex(warehouse => warehouse.id === id);
  if (index === -1) return { error: true, status: 404, message: 'Not found' } as ApiResponse<{ success: boolean }>;
  ...
  return { data: { success: true }, status: 200, error: false } as ApiResponse<{ success: boolean }>;
}

export function getWarehouseAuditLog (): WarehouseAuditLog[] { return warehouseAuditLog; }
...
export const mockInventoryData = [
  {
    ...
    warehouseId: 1,
    variants: [
      { id: 1, name: '128GB Titanio Natural', available: 25, storage: '128GB', color: 'Titanio Natural', price: '$1199.00' },
      { id: 2, name: '256GB Titanio Azul', available: 18, storage: '256GB', color: 'Titanio Azul', price: '$1299.00' }
    ],
  ...
  },
  {
    ...
    warehouseId: 1,
    variants: [
      { id: 3, name: 'M3 Pro, 512GB', available: 12, processor: 'M3 Pro', storage: '512GB', price: '$2199.00' }
    ],
