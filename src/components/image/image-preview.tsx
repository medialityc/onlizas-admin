...
const ImagePreview = ({
  ...
  return (
    <>
      <div
        className={cn(
          "object-contain w-14 h-14 shrink-0 bg-slate-100 border-slate-400 dark:border-slate-700 dark:bg-slate-700 rounded-md overflow-hidden relative",
          previewEnabled &&
            !showIcon &&
            !isDefaultImage &&
      ...
      {open && previewEnabled && !isDefaultImage && (
        <SimpleModal open={open} onClose={() => handlePreviewToggle(false)}>
          <div
                ...
                <div className="flex gap-2 mt-4 overflow-x-auto max-w-full px-4 py-2 pb-4 scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-transparent">
                  {imagesToShow?.map((img, idx) => (
                    <button
                      key={idx}
                      className={cn(
                        "w-16 h-16 border-2 rounded overflow-hidden shrink-0 transition-all duration-200",
                        idx === current
