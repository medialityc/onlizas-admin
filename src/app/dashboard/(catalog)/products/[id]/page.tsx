...
async function ProductDetails ({ id }: { id: string }) {
  ...
  const productImage = product.images?.find(img => img.order === 1)?.image || '/placeholder-product.jpg';
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-lg border p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Proveedores que usan este producto</h3>

        {product.suppliers && product.suppliers.length > 0 ? (
          <div className="space-y-3">
            {product.suppliers.map((supplier) => (
              <div key={supplier.id} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-b-0">
      ...
      {(product.about && product.about.length > 0) || (product.details && product.details.length > 0) ? (
        <div className="bg-white rounded-lg border p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Información adicional</h3>

          {product.about && product.about.length > 0 && (
            <div className="mb-6">
              <h4 className="font-medium text-gray-700 mb-2">Acerca del producto:</h4>
